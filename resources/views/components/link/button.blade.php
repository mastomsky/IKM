...
@else
  <a href="{{ $href }}" class="{{ $class }} font-medium rounded-lg  bg-{{ $color }}-700 hover:bg-{{ $color }}-800 focus:ring-4 focus:outline-none focus:ring-{{ $color }}-300 font-medium rounded-lg text-sm p-2.5 text-center inline-flex items-center mr-2 dark:bg-{{ $color }}-600 dark:hover:bg-{{ $color }}-700 dark:focus:ring-{{ $color }}-800" style="background: white !important">
    @if (!empty($icon))
      <x-icon :name="$icon" />
    @endif
    @if (!empty($text))
